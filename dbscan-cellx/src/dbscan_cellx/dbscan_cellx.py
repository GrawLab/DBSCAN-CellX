...
def edit_table(files, save, pixel_list, edge_mode, angel_paramter, save_paramter):
    for j in files:
        ...
        for i in table_all["ImageNumber"].unique():
            ...
            savepath = save + name + 'DBSCAN_CELLX_output.csv'
            if save_paramter == 1:
                savepath_para = save + name + "_paramter_list.csv"
        ...
        df_list = df_list.rename(
            columns={"Nucleus number_x": "Cell_ID", "Nucleus number_y": "Cells_in_Cluster", "corrected_cluster" : "Cluster_Position"})
        df_list.drop(df_list.columns.difference(
            ['ImageNumber', 'Cell_ID', "X", "Y", "Cluster_ID", "Cell_in_Image", "Cells_in_Cluster", "Cluster_Position", "Edge_Degree"]), 1, inplace=True)
        df_list.to_csv(savepath, sep=';', index=False)
        


def main(files, save, pixel_ratio, size_x, size_y, edge_mode = 1, angel_paramter = 140, save_paramter = 1):
    st = time.time()
    ...
    edit_table(files,save,
               pixel_list, edge_mode, angel_paramter, save_paramter)
    et = time.time()
    elapsed = et-st
